...
class Classifier_Train : IDisposable
{
    ...
    {
        Console.WriteLine(Directory.GetCurrentDirectory() + "\\FaceDB");
        _IsTrained = LoadTrainingData(Directory.GetCurrentDirectory() + "\\FaceDB");
    }
    ...
    public bool Retrain()
    {
        return _IsTrained = LoadTrainingData(Directory.GetCurrentDirectory() + "\\FaceDB");
    }
    /// <summary>
    /// Retrains the recognizer witout resetting variables like recognizer type.
    /// Takes String input to a different location for training data.
                            ...
                            switch (xmlreader.Name)
                            {
                                ...
                                case "FILE":
                                    if (xmlreader.Read())
                                    {
                                        ...
                                        trainingImages.Add(new Image<Gray, byte>(Directory.GetCurrentDirectory() + "\\TrainedFaces\\" + xmlreader.Value.Trim()));
                                    }
                                    break;
                            }
                        }
